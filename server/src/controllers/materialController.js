...
const deleteMaterial = async (req, res, next) => {
  try {
    ...
    if (!fileId || !fileURL) {
      return res
        .status(400)
        .json({ message: "Missing required parameters: fileId or fileURL." });
    }
    ...
    try {
      ...
      return res.status(404).json({
        message:
          "Failed to fetch file metadata from GitHub. File may not exist.",
    ...
    try {
      const response = await octokit.request(
        `DELETE /repos/${process.env.GITHUB_USER}/${process.env.REPO}/contents/${sanitizedPath}`,
        {
          owner: process.env.GITHUB_USER,
          repo: process.env.REPO,
    ...
    try {
      ...
      if (!deletedMaterial) {
        return res
          .status(404)
          .json({ message: "Material not found or unauthorized access." });
      }
    } catch (dbError) {
    ...
    res
      .status(200)
      .json({ message: "Material and file deleted successfully." });
  } catch (error) {
...
const chunkifyMaterial = async (req, res) => {
  try {
    ...
    if (!filePath) {
      return res
        .status(400)
        .json({ error: "No filePath provided in material" });
    }
    ...
    if (fileExtension === ".pdf") {
      ...
      [".doc", ".docx", ".ppt", ".pptx", ".txt"].includes(fileExtension)
    ) {
